...
export default function AIComposer() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
      ...
      <div className=" space-y-6 text-center mt-20">
        ...
        <div className="relative group mt-12">
          <textarea
            ...
            placeholder="Share your idea... (e.g. A post about the launch of our new eco-friendly technology)"
          />
          <div className="absolute bottom-4 right-2.5 flex items-center gap-3 text-sm">
            <button
              onClick={() => setGenerateImage(!generateImage)}
              className="flex  items-center gap-3 bg-red-50 py-2 px-3 rounded-lg cursor-pointer"
            >
              <span>AI Image</span>
              <div
            ...
            <button
              ...
              disabled={loading}
              className="bg-slate-900 cursor-pointer hover:bg-slate-800 text-white flex items-center gap-2 px-4 py-2 rounded-lg"
            >
              {loading ? (
                <>
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {tones.map((t) => (
            <button
              key={t}
              onClick={() => setTone(t)}
              className={`px-4 py-1.5 rounded-full text-sm transition-all border cursor-pointer ${t === tone ? "bg-red-500 border-red-500 text-white" : "bg-white border-slate-200 text-slate-500 hover:border-slate-300"}`}
            >
              {t}
            </button>
          ))}
              ...
              <div className="flex flex-col h-full space-y-4">
                ...
                <div className="flex items-center gap-2 pt-2">
                  <button
                    ...
                    className="flex-1 cursor-pointer bg-slate-100  hover:bg-red-500 hover:text-white text-slate-600  text-xs py-2.5 rounded-lg transition-all"
                  >
                    Schedule Post
                  </button>
                </div>
          ...
          {generations.length === 0 && (
            <div className="col-span-full py-20 text-center space-y-2">
              ...
              <p className="text-slate-400 text-sm">
                No content generated yet. Try generating some contednt using the
                AI.
              </p>
            </div>
          )}
      ...
      {/* Scheduler Modal  */}
      {activeScheduler && (
        <div className="fixed overflow-hidden  inset-0 min-h-screen z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between px-8 py-4 border-b  border-slate-100 bg-slate-50/30">
              ...
              <button
                ...
                className="cursor-pointer p-2 rounded-full hover:bg-slate-100 text-slate-400 transition-colors"
              >
                  ...
                  {PLATFORMS.map((p) => {
                    ...
                    return (
                      <button
                        ...
                        className={`p-2.5 cursor-pointer rounded-md border text-xs ${active ? "bg-red-500/80 text-white" : "bg-white border-slate-200  text-slate-400 hover:border-slate-300"}`}
                      >
            ...
            <button
              ...
              className="cursor-pointer w-full flex items-center justify-center gap-2 py-3  rounded-md bg-slate-200 text-slate-700 hover:bg-red-500 hover:text-white transition"
            >
              {scheduling ? (
                <Loader2Icon className="size-4 animate-spin" />
              ) : (
